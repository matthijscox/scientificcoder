...
Here are some of the interfaces that are important for initializing the Julia library in the correct manner for accepting/adopting external threads from C++. We requested advise to use many of these functions, as we're not experts in this either. The Julia C API could definitely use some more documentation.
...
* `jl_gc_safe_enter` sets a safe point for the garbage collector, ensuring that during program execution, all garbage collector roots are known and all heap object contents are consistent. From a global perspective, all threads will wait at this safe point before the garbage collector can run. The use of [parentheses around the function](https://stackoverflow.com/questions/13600790/what-do-the-parentheses-around-a-function-name-mean) is simply to avoid confusion with a function-like macro.
